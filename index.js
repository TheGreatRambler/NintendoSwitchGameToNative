...
async function startMcsema(game) {
	await new Promise(resolve => {
		...
		var cmd = `${res("mcsema/bin/mcsema-lift-5.0.exe")} --os linux --arch aarch64 --cfg ${res("games/" + game + "/config.cfg")} --output ${res("games/" + game + "/bitcode.bc")} --explicit_args --abi_libraries ${res("aarch64.bc")}`;
		console.log(cmd);
